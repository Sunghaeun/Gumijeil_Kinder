...
// "roster"/"attendance"는 연도별 구조 도입 이전의 예전 키(하위 호환용으로 계속 읽을 수
// 있게 허용). "roster_2026"/"attendance_2026"처럼 연도별로 나뉜 키와, 연도 목록/현재연도를
// 담는 "roster_meta"를 허용합니다. 반별명단뿐 아니라 출석부도 연도마다 사람 id가 다시
// 매겨지기 때문에 반드시 연도별로 나눠서 저장/조회해야 서로 안 섞입니다.
const STATIC_ALLOWED_KEYS = new Set(["roster", "attendance", "roster_meta"]);
const ROSTER_YEAR_KEY_RE = /^roster_\d{4}$/;
const ATTENDANCE_YEAR_KEY_RE = /^attendance_\d{4}$/;
const YEAR_KEY_RE = /^(roster|attendance)_\d{4}$/; // GET/PUT/DELETE 모두에 쓰이는 연도별 키 판별용

function isAllowedKey(key) {
  return typeof key === "string" && (STATIC_ALLOWED_KEYS.has(key) || ROSTER_YEAR_KEY_RE.test(key) || ATTENDANCE_YEAR_KEY_RE.test(key));
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "DELETE") {
    const key = req.query.key;
    // 삭제는 연도별 백업(roster_YYYY, attendance_YYYY)에 대해서만 허용합니다 (5년 지난 자료 자동 파기용).
    // "roster_meta"나 예전 방식의 "roster"/"attendance" 같은 핵심 키는 실수로라도 지울 수 없게 막습니다.
